  ...
  useEffect(() => {
    ...
    const resize = () => {
      try {
        ...
        const cssW = canvas.clientWidth || canvas.parentElement?.clientWidth || vw;
        const cssH = canvas.clientHeight || canvas.parentElement?.clientHeight || vh;
        const w = Math.max(1, Math.floor((cssW || vw) * dpr));
        const h = Math.max(1, Math.floor((cssH || vh) * dpr));
